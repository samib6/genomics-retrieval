...
def evaluate_kmer_task(task_key, k_values=(2, 3, 4), eval_at=(5, 10, 50)):
    ...
    for k in k_values:
        ...
        if k == 4:
            k4_ranked_lists = ranked_id_lists
    ...
    return results, query_ids, k4_ranked_lists

def save_kmer_predictions(query_ids, ranked_lists, path):
    import pandas as pd

    rows = []

    for qid, docs in zip(query_ids, ranked_lists):
        for rank, doc_id in enumerate(docs[:50], start=1):
            rows.append({
                "query_id": str(qid),
...
if __name__ == "__main__":
    ...
    for task in ("arch", "euk"):
        ...
        results, query_ids, k4_ranked_lists = evaluate_kmer_task(
            task,
            k_values=(2, 3, 4),
            eval_at=(5, 10, 50)
        )
        ...
        # SAVE k=4 predictions
        if task == "arch":
            save_kmer_predictions(
                query_ids,
                k4_ranked_lists,
                "kmer_k4_top50_arch.csv"
            )
